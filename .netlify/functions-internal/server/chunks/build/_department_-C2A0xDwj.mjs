  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_UBadge, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`${ssrInterpolate(aiPlatforms.length)} 个可用 `);
          } else {
            return [
              createTextVNode(toDisplayString(aiPlatforms.length) + " 个可用 ", 1)
            ];
          }
        }),
        _: 1
      ...
      if (currentPlatformInfo.value) {
        ...
        _push(`</div></div><div class="p-3 bg-gray-50 rounded-lg"><h5 class="text-sm font-medium text-gray-700 mb-2">快速操作</h5><div class="space-y-2">`);
        _push(ssrRenderComponent(_component_UButton, {
          variant: "ghost",
          size: "sm",
          icon: "i-heroicons-arrow-path",
      ...
      _push(`</div></aside><div class="flex-1 bg-white/80 backdrop-blur-md rounded-xl shadow-lg border border-white/20 flex flex-col"><div class="p-4 border-b border-gray-200 flex-shrink-0"><div class="flex items-center justify-between"><div class="flex items-center gap-3"><div><h2 class="text-lg font-semibold text-gray-800">AI助手界面</h2><p class="text-sm text-gray-600">${ssrInterpolate(proxyUrl.value ? currentPlatformInfo.value?.label || "加载中..." : "请选择平台开始使用")}</p></div></div><div class="flex items-center gap-4"><div class="flex items-center gap-2"><div class="${ssrRenderClass(iframeStatusClass.value)}"></div><span class="text-sm text-gray-600">${ssrInterpolate(iframeStatus.value)}</span></div>`);
      ...
      if (isLoading.value) {
        ...
        _push(`<p class="text-sm text-gray-600 mb-2">正在加载 ${ssrInterpolate(currentPlatformInfo.value?.label)}...</p><div class="w-32 h-1 bg-gray-200 rounded-full overflow-hidden mx-auto"><div class="h-full bg-blue-500 rounded-full animate-pulse" style="${ssrRenderStyle({ "width": "60%" })}"></div></div></div></div>`);
        ...
        _push(`<h3 class="text-lg font-medium text-gray-800 mb-2">加载失败</h3><p class="text-sm text-gray-600 mb-4">${ssrInterpolate(errorMessage.value)}</p><div class="flex gap-2 justify-center">`);
        _push(ssrRenderComponent(_component_UButton, {
          variant: "outline",
          size: "sm",
          onClick: refreshPage
      ...
      _push(`<iframe style="${ssrRenderStyle(proxyUrl.value && !isLoading.value && !hasError.value ? null : { display: "none" })}"${ssrRenderAttr("src", proxyUrl.value)} class="w-full h-full border-0"${ssrRenderAttr("title", selectedPlatform.value ? `${selectedPlatform.value} 平台页面` : "AI平台页面")} sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-top-navigation allow-downloads"></iframe>`);
      if (!proxyUrl.value && !isLoading.value) {
        ...
        _push(`<p class="text-lg font-medium mb-2">请选择AI平台</p><p class="text-sm mb-4">从左侧选择一个AI平台开始使用</p>`);
        if (aiPlatforms.length > 0) {
          _push(ssrRenderComponent(_component_UButton, {
            variant: "soft",
            size: "sm",
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(` 选择 ${ssrInterpolate(aiPlatforms[0]?.label)}`);
              } else {
                return [
                  createTextVNode(" 选择 " + toDisplayString(aiPlatforms[0]?.label), 1)
                ];
              }
            }),
            _: 1
